...
class UploadError(Exception):
    ...
    report the same taxonomy the rest of the server uses.
    """

    def __init__(self, message: str, error_type: str = "InvalidParameter") -> None:
...
class UploadStore:
    ...
    def __init__(self, directory: Path, *, max_bytes: int, ttl_seconds: int) -> None:
        self._dir = directory
    ...
    def _create(self, data: bytes, *, filename: str, more_chunks: bool) -> dict[str, Any]:
        if len(data) > self._max_bytes:
            raise UploadError(
                f"Upload is {len(data)} bytes, over the {self._max_bytes} byte limit. "
                f"Raise {MAX_UPLOAD_BYTES_ENV} or trim the capture with editcap.",
                "InvalidParameter",
            )
        if not more_chunks and detect_capture_format(data[:4]) is None:
            raise UploadError("Content is not a pcap or pcapng capture (magic number check failed).")
        ...
        try:
            ...
            with os.fdopen(fd, "wb") as handle_out:
                handle_out.write(data)
        except OSError as exc:
            raise UploadError(f"Could not write upload: {exc}", "PermissionDenied") from exc
    ...
    def _append(self, upload_id: str, data: bytes, *, more_chunks: bool) -> dict[str, Any]:
        meta = self._read_meta(upload_id)
        if meta is None:
            raise UploadError(f"Unknown or expired upload: {UPLOAD_SCHEME}{upload_id}", "FileNotFound")
        ...
        if size + len(data) > self._max_bytes:
            ...
            self._remove(upload_id)
            raise UploadError(
                f"Upload would reach {size + len(data)} bytes, over the {self._max_bytes} byte limit. "
                f"The partial upload was discarded. Raise {MAX_UPLOAD_BYTES_ENV} or split the capture.",
                "InvalidParameter",
            )

        part_path = self._dir / f"{upload_id}.part"
        try:
            with open(part_path, "ab") as handle_out:
                handle_out.write(data)
        except OSError as exc:
            raise UploadError(f"Could not append to upload: {exc}", "PermissionDenied") from exc

        meta["size"] = size + len(data)
        self._write_meta(upload_id, meta)
    ...
    def _finalize(self, upload_id: str, meta: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            self._remove(upload_id)
            raise UploadError(f"Could not read upload: {exc}", "PermissionDenied") from exc

        capture_format = detect_capture_format(head)
        if capture_format is None:
            self._remove(upload_id)
            raise UploadError("Assembled content is not a pcap or pcapng capture (magic number check failed).")
        ...
        try:
            ...
            self._remove(upload_id)
            raise UploadError(f"Could not finalize upload: {exc}", "PermissionDenied") from exc
...
def create_upload_store(allowed_dirs: list[str] | None) -> UploadStore:
    ...
    logger.info(
        "Capture uploads enabled at %s (max %d bytes, TTL %d seconds)",
        resolved,
        store.max_bytes,
        store.ttl_seconds,
    )
